...
class Application
{
    ...
    public function executeHttpRequest(): array
    {
        ...
        if(!method_exists($controller,$response["method"])) {
            ...
            $httpResponse->setMessage("Method handler not found for '".$response["method"]."' in controller '".$response["controller"]."'");
        ...
        if($argCount !== $varCount){
            $httpResponse->setOutcome(false);
            $httpResponse->setStatusCode(500);
            $httpResponse->setMessage("Method handler error, argument mismatch, required arguments = ".$varCount.", provided = ".$argCount);
            return $httpResponse->getArray();
        }
